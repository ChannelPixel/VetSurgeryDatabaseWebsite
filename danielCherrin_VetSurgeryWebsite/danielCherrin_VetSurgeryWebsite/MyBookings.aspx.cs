...
public partial class MyBookings : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                string str_queryMyBookings = null;

                if ((string)Session["user_type"] == "Vet")
                {
                    str_queryMyBookings = "SELECT *" +
                ...
                try
                {
                    SqlDataAdapter ada_allBookings = new SqlDataAdapter(str_queryMyBookings, sqlConn.conn);
                    DataSet ds_allBookings = new DataSet();
                    ada_allBookings.Fill(ds_allBookings);
                    gv_myBookings.DataSource = ds_allBookings.Tables[0];
                    gv_myBookings.DataBind();
                }
                catch (Exception ex)
                {
    ...
    protected void gv_myBookings_RowDataBound(object sender, GridViewRowEventArgs e)
    {
    ...
    protected void gv_myBookings_RowCommand(object sender, GridViewCommandEventArgs e)
    {
        if (((Button)e.CommandSource).ID == "gvBtnEdit")
        {
            GridViewRow gvr = (GridViewRow)(((Button)e.CommandSource).NamingContainer);

            Session["editBookingId"] = gv_myBookings.DataKeys[gvr.RowIndex].Values[0].ToString();
            Session["editBookingVet"] = gv_myBookings.Rows[gvr.RowIndex].Cells[1].Text.ToString();
            Session["editBookingPetId"] = gv_myBookings.DataKeys[gvr.RowIndex].Values[1].ToString();
        ...
        if (((Button)e.CommandSource).ID == "gvBtnSelect")
        {
            ...
            Session["selectPetId"] = gv_myBookings.DataKeys[gvr.RowIndex].Values[1].ToString();
            ...
            Session["selectBookingId"] = gv_myBookings.DataKeys[gvr.RowIndex].Values[0].ToString();
